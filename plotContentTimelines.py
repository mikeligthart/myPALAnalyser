...
for participant_index in range(0, numberOfParticipants):
    ...
    median = np.median(y[np.nonzero(y)])
    iqr = np.subtract(*np.percentile(y[np.nonzero(y)], [75, 25]))    
    total = np.sum(y)
    ...
    median_daily = np.median(y_daily[np.nonzero(y_daily)])
    iqr_daily = np.subtract(*np.percentile(y_daily[np.nonzero(y_daily)], [75, 25]))    
    ...
    if len(y_total_nonzero) > 0:
        median_total = np.median(y_total_nonzero)
        iqr_total = np.subtract(*np.percentile(y_total_nonzero, [75, 25]))
    else:
        median_total = 0
        iqr_total = 0
    total_total = np.sum(y_total)
    total_combined = total_daily + total_total
    text = '    Daily Goal    Total Goal \nmedian     ' + str(median_daily) + '    '+ str(median_total) +' \nIQR    '+ str(iqr_daily) +' \t ' + str(iqr_total) + ' \ntotal ' + str(total_daily) + ' \t '+ str(total_total)
    ...
    lgd3 = ax3.legend(prop = font_prop, loc='upper right', bbox_to_anchor=(1.4, 1.25), title='Goals')
    metrics3 = f.text(1.005, 0.15, text, size=7)
